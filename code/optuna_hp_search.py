...
def run(trial):
    ...
    sub_units = trial.suggest_int("sub_units", 512, 2048, log=True)
    sub_drop = trial.suggest_float("sub_drop", 0.2, 0.8, log=True)

    model = HpaModel(classes = 19, device = device, 
                            base_model_name = 'resnest50', 
                            features = 2048, pretrained = True,sub_units = sub_units, sub_drop = sub_drop,)
