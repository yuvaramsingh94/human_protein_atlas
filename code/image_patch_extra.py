...
train_df = pd.read_csv('data/train_ext/extra_cutdown_data.csv')
...
def img_splitter(im_tok):
    ...
    #print(im_tok)
    img_red    = np.expand_dims(imageio.imread(os.path.join('data/train_ext/HPA-Challenge-2021-trainset-extra',f'{im_tok}_red.png')), axis = -1)
    img_yellow = np.expand_dims(imageio.imread(os.path.join('data/train_ext/HPA-Challenge-2021-trainset-extra',f'{im_tok}_yellow.png')), axis = -1)
    img_green  = np.expand_dims(imageio.imread(os.path.join('data/train_ext/HPA-Challenge-2021-trainset-extra',f'{im_tok}_green.png')), axis = -1)
    img_blue   = np.expand_dims(imageio.imread(os.path.join('data/train_ext/HPA-Challenge-2021-trainset-extra',f'{im_tok}_blue.png')), axis = -1)
    image = np.concatenate([img_red, img_yellow, img_green, img_blue], axis=-1)
    img_mask = np.load(os.path.join('data/train_ext/mask',f'{im_tok}.npz'))['arr_0'].astype(np.uint8)
