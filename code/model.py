...
class Autopool(nn.Module):
    ...
    def forward(self, x):
        ...
        final_out = torch.sum(torch.mul(x, weights), dim=1)
...
class Autopool_1(nn.Module):
    def __init__(
        self,
        input_size,
        device,
    ):
        super(Autopool_1, self).__init__()
        self.alpha = nn.Parameter(requires_grad=True)
        self.alpha.data = torch.zeros(
            [input_size], dtype=torch.float32, requires_grad=True, device=device
        )
        self.sigmoid_layer = nn.Sigmoid()
...
class HpaModel_1(nn.Module):
    def __init__(self, classes, device, base_model_name, pretrained, features, init_linear_comb = False):
        ...
        self.model = nn.Sequential(*layers)
        self.fc = HpaSub(classes, features)
        self.autopool = Autopool_1(input_size = classes, device = device)
